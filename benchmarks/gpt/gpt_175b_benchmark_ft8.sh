...
do
    ...
    logdir="perf/fp8/gpt-TP${tensor_para_size}-PP${pipeline_para_size}-log"
    if [ ! -f ${logdir} ]; then
	mkdir ${logdir} -p
    fi
	...
	do
		...
		do
		    ...
		    mpirun -n ${total_gpu_count} --allow-run-as-root ./bin/gpt_fp8_example .tmp.config.ini 2>&1 | tee ${tmp_log}
