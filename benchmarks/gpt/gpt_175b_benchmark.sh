...
do
	...
	do
		...
		do
		    ...
		    #FT_NVTX=ON nsys profile -s none -t cuda,nvtx,osrt --force-overwrite=true -o ${logdir}/bs-${request_batch_size}-int8_mode-${int8_mode}-layers-${num_layer} --capture-range=cudaProfilerApi --capture-range-end=stop  mpirun -n ${total_gpu_count} --allow-run-as-root ./bin/multi_gpu_gpt_example .tmp.config.ini 2>&1 | tee ${tmp_log}
